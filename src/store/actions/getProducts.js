...
export const getProducts = (delivery) => async dispatch => {
  try {
    let products
    if (delivery === true) {
      products = await axios.get(`https://fast-food-server.herokuapp.com/api/products/?delivery=true`)
    } else {
      products = await axios.get(`https://fast-food-server.herokuapp.com/api/products`)
    }

    const categories = await axios.get('https://fast-food-server.herokuapp.com/api/categories')
